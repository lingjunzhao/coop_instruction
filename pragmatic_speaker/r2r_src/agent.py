...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20, seed=1):
        ...
        if args.optim == 'adafactor':
            self.encoder_optimizer = args.optimizer(self.encoder.parameters(), scale_parameter=False, relative_step=False, warmup_init=False, lr=args.lr)
            self.decoder_optimizer = args.optimizer(self.decoder.parameters(), scale_parameter=False, relative_step=False, warmup_init=False, lr=args.lr)
            self.critic_optimizer = args.optimizer(self.critic.parameters(), scale_parameter=False, relative_step=False, warmup_init=False, lr=args.lr)
        else:
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        if speaker is not None:         # Trigger the self_train mode!
            ...
            insts, probs = speaker.infer_batch(featdropmask=noise)     # Use the same drop mask in speaker
