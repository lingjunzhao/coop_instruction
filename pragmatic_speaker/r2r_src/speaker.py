...
class Speaker():
    ...
    def get_insts(self, wrapper=(lambda x: x), sampling=False, decode_temp=1.0, decode_top_p=1.0):
        ...
        for _ in wrapper(range(total // self.env.batch_size + 1)):  # Guarantee that all the data are processed
            obs = self.env.reset()
            insts, log_probs = self.infer_batch(sampling=sampling)  # Get the insts of the result
    ...
    def infer_batch(self, sampling=False, train=False, featdropmask=None):
        ...
        if train and sampling:
            ...
            return np.stack(words, 1), torch.stack(log_probs, 1)      # [(b), (b), (b), ...] --> [b, l]
