...
from utils import load_datasets, load_nav_graphs, Tokenizer, load_speaker_outputs
...
class R2RBatch():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
                 name=None, train_sampling=1.0, use_generated_instr=False, speaker_outputs=False):
        ...
        if speaker_outputs:
            ...
            for split in splits:
                count = 0
                for item in load_datasets([split], name=name):
                    # Split multiple instructions into separate entries
                    if not use_generated_instr:
        ...
        if self.name == 'train' or self.name == 'aug':
            if train_sampling < 1.0:
                len_data = len(self.data)
                print("Original training size: ", len_data)
                num_samples = int(len_data * float(train_sampling))
        ...
        # self.data = self.data[:200]  # TODO: delete
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            obs.append({
                ...
                'instr_tokens' : item['instructions'],
                'scan' : state.scanId,
